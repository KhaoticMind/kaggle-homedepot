...
def get_w2v_ratio(w2v, text1, text2, n_grams_1=1, n_grams_2=1, n_jobs=1):
...
def w2v(df_all, cols):
    ...
    for col in cols:
        for i in range(3):
            for j in range(3):
                df['w2v_' + col + str(i) + '_' + str(j)] = df_all.apply(axis=1, func=lambda x: get_w2v_ratio(w2v, x['search_term'], x[col], i, j))    
...
if __name__ == '__main__':
    ...
    del df_all, df_corrected, df_stemmed, df_char, df_word, df_gram                    
    ...
    df_train = df_tudo.iloc[:num_train]
    df_test = df_tudo.iloc[num_train:]
    id_test = df_test['id']
    y_train = df_train.relevance.values
    
    df_train = df_metrics.iloc[:num_train]
    df_test = df_metrics.iloc[num_train:]
    
    #df_train.drop( ['id', 'product_uid', 'relevance'])
